...
def get_rsa(msg: bytes, sk: rsa.SecretKey) -> bytes:
    masked = oaep.mask(msg)
    res = rsa.process(int.from_bytes(masked, 'big'), sk)
...
if __name__ == '__main__':
    ...
    rsa_msg = get_rsa(hash + key + nonce_bytes, sk) # len(hash+key+nonce) == 96 bytes, 768 bits
    ...
    if args.key:
        with open(args.key, 'wb') as f:
            n = int.to_bytes(pk.n, 2048 // 8, 'big')
            d = int.to_bytes(pk.e, 2048 // 8, 'big')
